...
#   npx supabase start
#   npx supabase test db        # runs supabase/wepatent/tests/*.sql with pgTAP
#
...
psql -d "$DB" -q -v ON_ERROR_STOP=1 -f supabase/wepatent/tests/helpers/local_auth_shim.sql

echo "== Applying migrations"
for f in supabase/wepatent/migrations/0*.sql; do
  echo "   -> $f"
  psql -d "$DB" -q -v ON_ERROR_STOP=1 -f "$f"
done

echo "== Applying Supabase-equivalent grants + pgTAP + fixture"
psql -d "$DB" -q -v ON_ERROR_STOP=1 -f supabase/wepatent/tests/helpers/grants.sql
...
psql -d "$DB" -q -v ON_ERROR_STOP=1 -f supabase/wepatent/tests/helpers/seed.sql

echo "== Running pgTAP suite"
pg_prove -d "$DB" --ext .sql supabase/wepatent/tests/0*.sql
