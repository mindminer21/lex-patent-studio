...
psql -d "$DB" -q -v ON_ERROR_STOP=1 -f supabase/lex/tests/helpers/grants.sql
...
psql -d "$DB" -q -v ON_ERROR_STOP=1 -f supabase/lex/tests/helpers/seed.sql

echo "== [private] Running pgTAP suite"
pg_prove -d "$DB" --ext .sql supabase/lex/tests/0*.sql

echo "== [corpus] Applying corpus migrations (separate project, separate DB)"
for f in supabase/lex/corpus-migrations/0*.sql; do
  echo "   -> $f"
...
pg_prove -d "$CORPUS_DB" --ext .sql supabase/lex/tests/corpus/0*.sql
