...
./scripts/test-rls-wepatent.sh          # RLS pgTAP matrix on a throwaway PostgreSQL 16
...
1. **Supabase**: create the private application project AND a separate corpus project.
   Apply `supabase/wepatent/migrations/*` to the private project and `supabase/wepatent/corpus/migrations/*`
...
| Suspected cross-tenant issue | Check `authz.denied` counter and `api.*` warn logs (correlation IDs); re-run `./scripts/test-rls-wepatent.sh`; RLS denials in production appear as empty results, never foreign rows. |
