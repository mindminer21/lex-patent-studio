...
test("wallet top-up: checkout handoff → signed webhook → ledger credit", async ({ page }) => {
  ...
  await page.waitForURL(/\/wepatent\/app\/billing\/simulated-checkout/);
...
test("customer portal handoff reaches the labeled simulated portal", async ({ page }) => {
  ...
  await page.waitForURL(/\/wepatent\/app\/billing\/simulated-portal/);
  ...
  await page.waitForURL(/\/wepatent\/app\/billing/);
});
